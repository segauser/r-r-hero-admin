...
// // библиотека TransitionGroup с анимацией при удалении
...
const HeroesList = () => {
    ...
    const filteredHeroesSelector = createSelector(
        (state) => state.filters.activeFilter, //1 ф-я
        (state) => state.heroes.heroes, // 2 ф-я
        // результат 1 и 2 ф-ий, здесь filter = state.filters.activeFilter
        (filter, heroes) => {
            if (filter === 'all') {
                console.log('render');
                return heroes;
